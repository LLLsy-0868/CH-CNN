...
class StatusUpdateTool(object):
    ...
    @classmethod
    def get_min_complexity(cls):
        rs = cls.__read_ini_file('network', 'min_complexity')
        return int(rs)
    ...
    def get_init_params(cls):
        ...
        params['min_complexity'] = cls.get_min_complexity()
        return params
...
class Utils(object):
    ...
    def load_cache_data(cls):
        ...
        if os.path.exists(file_name):
            ...
            for each_line in f:
                ...
                list = rs_[1:-1]
                list[0] = '%.5f' % (float(rs_[1]))
                _map[rs_[0]] = list
            f.close()
        return _map

    @classmethod                                                                                          ###########
    def save_fitness_to_cache(cls, individuals,gen_no):  # 增加存储个体 cv, complexity
        print("save_fitness_to_cache")
        _map = cls.load_cache_data()  # map字典对应2个值 acc, complexity
        for indi in individuals:
            _key, _str = indi.uuid()
            _acc = indi.acc
            _mean = indi.mean
            _cv = indi.cv
            _complexity = int(indi.complexity)
            if _key not in _map:
                Log.info('Add record into cache, id:%s, acc:%.5f, cv:%d, complexity:%d' % (
                _key, _acc, _cv, _complexity))
                f = open('./populations/cache.txt', 'a+')
                str = '%s;%.5f;%d;%s\n' % (_key, _acc, _complexity, _str)
                f.write(str)
                f.close()
            else:
                if indi.change == 1 or gen_no > 9:
                    Log.info('Change record into cache, id:%s, acc:%.5f, cv:%d, complexity:%d' % (
                        _key, _acc, _cv, _complexity))
                    with open("./populations/cache.txt", "r") as f:
                        lines = f.readlines()

                    with open("./populations/cache.txt", "w") as f_w:
                        for line in lines:
                            if _key in line:
                                print(line)
                                str = '%s;%.5f;%d;%s\n' % (_key, _acc,_complexity, _str)
    ...
    @classmethod  ###########
    def save_indi_to_cache(cls, _key, _str, _acc, _complexity):  # 增加存储个体 cv, complexity
        print("save_indi_to_cache")
        _map = cls.load_cache_data()  # map字典对应2个值 acc, complexity
        if _key not in _map:
            Log.info('Add record into fitness, id:%s, acc:%.5f, complexity:%d' % (
                    _key, _acc, _complexity))
            f = open('./populations/cache.txt', 'a+')
            str = '%s;%.5f;%d;%s\n' % (_key, _acc, _complexity, _str)
            f.write(str)
            f.close()
            ...
            # if change == 1:
                Log.info('Change record into fitness, id:%s, acc:%.5f, complexity:%d' % (
                        _key, _acc, _complexity))
                with open("./populations/cache.txt", "r") as f:
                        lines = f.readlines()
        ...
        while indi_start_line.startswith('indi'):
            ...
            for line in f:
                ...
                if line.startswith('--'):
                    ...
                    if line.startswith('Acc'):
                        ...
                        for data_item in data_maps:
                            ...
                            if _key == 'number':
                                ...
                                conv_params['type'] = int(_value)

                            elif _key == 'out':
                                conv_params['out_channel'] = int(_value)
                            else:
                                raise ValueError('Unknown key for load conv unit, key_name:%s' % (_key))
                        conv = ResUnit(conv_params['number'], conv_params['type'],conv_params['in_channel'], conv_params['out_channel'])
                    ...
                    elif indi_start_line.startswith('Pop_Min_Com'):
                        pop.min_complexity = int(indi_start_line[12:])
                    else:
            ...
            if os.path.exists(after_file_path):
                ...
                for indi in pop.individuals:
                    if indi.id in fitness_map:
                        ...
                        indi.complexity = fitness_map[indi.id][1]
    ...
    def generate_pytorch_file(cls, indi):
        ...
        for i, u in enumerate(indi.units):
            if u.type == 1:
                conv_name = 'self.conv%d_%d_%d_%d' % (i,u.block_type,u.in_channel, u.out_channel)
                if conv_name not in conv_name_list:
                    conv_name_list.append(conv_name)
                    if u.block_type ==0:
                        conv = '%s = Basic(in_planes=%d, planes=%d)' % (conv_name, u.in_channel, u.out_channel)
                    elif u.block_type ==1:
                        conv = '%s = BasicBlock(in_planes=%d, planes=%d)' % (conv_name, u.in_channel, u.out_channel)
                    elif u.block_type ==2:
                        conv = '%s = Bottleneck(in_planes=%d, planes=%d)' % (conv_name, u.in_channel, u.out_channel)
                    elif u.block_type == 3:
                        conv = '%s = ResNeXt0(in_planes=%d, planes=%d)' % (conv_name, u.in_channel, u.out_channel)
                    elif u.block_type == 4:
                        conv = '%s = ResNeXt1(in_planes=%d, planes=%d)' % (conv_name, u.in_channel, u.out_channel)
                    conv_list.append(conv)
                    se = 'self.SE_%d = SE_Block(ch_in=%d)'% (i,u.out_channel)
                    conv_list.append(se)
        out_channel_list = []
        ...
        for i, u in enumerate(indi.units):
            ...
            if u.type == 1:
                _str = 'out_%d = self.conv%d_%d_%d_%d(%s)' % (i, i, u.block_type,u.in_channel, u.out_channel, last_out_put)
                forward_list.append(_str)
                _str = 'out_%d = self.SE_%d(out_%d)' % (i,i,i)
                forward_list.append(_str)
            else:
                if u.max_or_avg < 0.5:
    ...
    @classmethod
    def new_generate_pytorch_file(cls, indi):
        # query convolution unit
        conv_name_list = []
        conv_list = []
        for u in indi.units:
