...
class ResUnit(Unit):
    def __init__(self, number,type, in_channel, out_channel): #prob < 0.5
        super().__init__(number)
        self.type = 1
        self.block_type = type #random.randint(0, 4)
...
class Individual(object):
    def __init__(self, params, indi_no):
        ...
        self.block_type = random.randint(0, 4)
        self.units = []
        self.change = 0
    ...
    def init_a_conv(self, _number, _in_channel, _out_channel):
        ...
        conv = ResUnit(number, self.block_type, _in_channel, out_channel)
        return conv
    ...
    def uuid(self):
        ...
        for unit in self.units:
            _sub_str = []
            #_sub_str.append('indi:%s'%(self.id))
            if unit.type == 1:
                _sub_str.append('conv')
                _sub_str.append('number:%d'%(unit.number))
                _sub_str.append(('type:%d'%(unit.block_type)))
    ...
    def __str__(self):
        ...
        _str.append('Complexity:%d'%(int(self.complexity)))
...
class Population(object):
    def __init__(self, params, gen_no):
        ...
        self.params = params
        self.individuals = []
        self.min_complexity = params['min_complexity']
    ...
    def initialize(self):
        for _ in range(self.pop_size):
            ...
            while param <= 0.1*self.min_complexity or param > 10000000:
                indi = Individual(self.params, indi_no)
                indi.initialize()
                net = cnn.EvoCNNModel(indi)
                param = utils.StatusUpdateTool.get_total_params(net)

            print("indi_param",param)
    ...
    def __str__(self):
        ...
        for ind in self.individuals:
            _str.append(str(ind))
            _str.append('-'*100)
        _str.append('Pop_Min_Com:%d' % (self.min_complexity))
        return '\n'.join(_str)
