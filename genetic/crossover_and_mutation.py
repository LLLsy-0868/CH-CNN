...
class Mutation(object):
    ...
    def do_modify_conv_mutation(self, indi):
        ...
        if len(conv_index_list) == 0:
            ...
            block_type = [0,1,2,3]
            index_ = int(np.floor(np.random.random() * len(block_type)))
            if indi.units[conv_index_list[selected_index]].block_type != block_type[index_]:
                indi.reset_acc()
                if selected_index > 0:
                    self.log.info('Unit at %d changes its block_type from %d to %d' % (
                    conv_index_list[selected_index], indi.units[conv_index_list[selected_index]].block_type,
                    block_type[index_]))
                    indi.units[conv_index_list[selected_index]].block_type = block_type[index_]

                else:
                    self.log.warn('Mutation position is 0, the block_type should not be changed')
            else:
                self.log.info('Unit at %d changes its block_type from %d to %d' % (
                conv_index_list[selected_index], indi.units[conv_index_list[selected_index]].block_type,
                block_type[index_]))
            ...
            if indi.units[conv_index_list[selected_index]].in_channel != channel_list[index_]:
                ...
                if selected_index > 0:
                    ...
                    indi.units[conv_index_list[selected_index-1]].out_channel = indi.units[conv_index_list[selected_index]].in_channel
                else:
            ...
            for i in range(len(conv_index_list)):
                if utils.flip(0.2):
                    ...
                    if indi.units[conv_index_list[i]].block_type != block_type:
                        self.log.info('Unit at %d changes its block type from %d to %d' % (conv_index_list[i], indi.units[conv_index_list[i]].block_type, block_type))
